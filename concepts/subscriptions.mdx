...
```mermaid
graph TD
    [Incomplete] --> |First payment succeeds| [Active]
    [Active] --> |Payment fails| --> [Past Due]
    [Past Due] --> |Credit card expired or reported stolen --> [Error]
    [Error] --> |Max retries exceeded --> [Expired]
    [Active] --> |Customer or admin pauses/cancels| --> [On Hold]
    [On Hold] --> |Customer resumes| --> [Active]
    [Past Due] --> |Payment succeeds| --> [Active]
```
