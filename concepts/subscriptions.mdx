...
Subscriptions progress through clearly defined states with automatic transitions:

```mermaid
graph TD
    A[Incomplete] -->|First payment succeeds| B[Active]
    B -->|Payment fails| C[Past Due]
    C -->|Technical error| D[Error]
    D -->|Max retries exceeded| E[Expired]
    B -->|Customer pauses| F[On Hold]
    F -->|Customer resumes| B
    B -->|Customer cancels| G[Cancelled]
...
<AccordionGroup>
  <Accordion title="Incomplete" icon="circle-pause">
    Subscription created but missing payment information. No charges attempted until payment method is added.
  </Accordion>
  ...
  <Accordion title="Past Due" icon="circle-exclamation">
    A recent payment failed with a retryable error (insufficient funds, card declined). The dunning process has started and will retry daily. Subscription remains functional during grace period.

    **Trigger:** Payment declined with error codes like `insufficient_funds`, `card_declined`, `do_not_honor`
  </Accordion>

  <Accordion title="Error" icon="circle-xmark">
    Payment failed with a non-retryable error (expired card, invalid card, fraud detection). Requires manual intervention or customer action to update payment method.

    **Trigger:** Technical payment errors or card issues that won't resolve with retries
  </Accordion>

  <Accordion title="On Hold" icon="pause">
    The subscription is temporarily paused by customer or merchant. No new orders or charges occur. Can be resumed at any time without losing configuration.

    **Use Cases:** Vacation, temporary financial hardship, seasonal pause, product out of stock
  </Accordion>

  <Accordion title="Expired" icon="ban">
    The subscription has been automatically cancelled after extended payment failure (exceeding `failed_payment_cancelled_days`, default 20 days). All pending deliveries are cancelled.

    **Final State:** Cannot be reactivated
  </Accordion>

  <Accordion title="Cancelled" icon="circle-minus">
    The subscription has been explicitly cancelled by the customer or merchant. No further processing occurs.
...
<CardGroup cols={2}>
  ...
  >
    Understand delivery scheduling and synchronization
  </Card>
</CardGroup>
