...
```mermaid
graph TD
    A[Day 1: Payment Fails] -->|Friendly reminder| B[Days 2-19: Payment Retries]
    B -->|Every 4th attempt| C[Day 4, 8, 12, 16: Urgent Reminders]
    C --> D[Day 20: Final Notice]
    D --> E[Day 21: Subscription Cancelled]

    B -.->|Payment succeeds| F[Back to Active]
    C -.->|Customer updates card| F
```

### Dunning Events

<AccordionGroup>
  <Accordion title="Past due first time" icon="circle-exclamation">
  ...
  <Accordion title="Still on error every fourth" icon="triangle-exclamation">
  ...
  <Accordion title="Error first time" icon="xmark">
  ...
  <Accordion title="Subscription expired" icon="ban">
